...
|C4|トリマコンデンサ (5-20pF)|SUNTAN TECHNOLOGY|[TSC3S520T1R](https://akizukidenshi.com/catalog/g/g113742/)|表面実装|1|
...
|R2|カーボン抵抗 (1kΩ)|KOA|[RK73B1JTTD102J](https://akizukidenshi.com/catalog/g/g130351/)|表面実装 (1608)|1|
...
- SW3: SW4の入り切りでリセットが可能なので、特に必要なければ実装しなくて構わない。J2にコネクタを接続すると干渉する
- C4: J2にコネクタを接続すると干渉するため、スルーホールから表面実装に変更 (裏面のパッドも忘れた)
...
|ID|種別|メーカー|型番|パッケージ|数量|
|:--|:--|:--|:--|:--|--:|
|R1|カーボン抵抗 (560Ω)|KOA|[RK73B1JTTD561J](https://www.digikey.jp/ja/products/detail/koa-speer-electronics-inc/RK73B1JTTD561J/9844753)|表面実装 (1608)|1|
...
- **TFカード読み書き デバッグ中**
  - CMD0でのソフトウェアリセットが応答しない、ACMD41での初期化が完了しない等 (カードによって挙動が変わる)

### ファームウェアの書き込み

自分でファームウェアを書き換える時のみ行う

- 基本的にはST-Linkで書き込む
  - [STLINK-V3MINIE](https://www.st.com/ja/development-tools/stlink-v3minie.html) が比較的安価 ([秋月電子でも入手可能](https://akizukidenshi.com/catalog/g/g118187/))
  - STM32 Nucleo付属のST-Linkから配線を引き出しても構わない
  - J-Linkでも書き込めるはず (未確認)
- 配線は次の通り
  - ピンヘッダJ2は右下が1番ピン
    - コネクタを繋いだ時にSW3やC4と干渉する場合、[1.27mmピッチ2x5ピンソケット](https://akizukidenshi.com/catalog/g/g113806/) を間に取り付ける
  - SWOは使用しない

|ピン割り当て|STLINK-V3MINIE|ピンヘッダJ2|
|:--|--:|--:|
|VTref|3|1|
|SWDIO|4|2|
|SWDCLK|6|4|
...
- CMakeまたは[STM32CubeIDE](https://www.st.com/ja/development-tools/stm32cubeide.html)でビルドしたのち書き込む
  - ビルド済みのelfファイルは[STM32CubeProgrammer](https://www.st.com/ja/development-tools/stm32cubeprog.html)でも書き込める
...
- スライドスイッチSW4を動かして電源を入れる
  ...
  - TFカード未挿入で電源を入れたり、読み込めない状態だと、**緑色LED**が点灯する
- 赤色LEDと緑色LEDの両方が消灯していれば、初期化は完了

### 初期設定

- USB Type-CポートでPCと接続する
  - CH340を認識しない場合、WCHのWebサイトからドライバ ([Windows](https://www.wch-ic.com/downloads/CH341SER_EXE.html)/[macOS](https://www.wch-ic.com/downloads/CH341SER_MAC_ZIP.html)/[Linux](https://www.wch-ic.com/downloads/CH341SER_LINUX_ZIP.html)) をインストールする
- シリアル通信 (115200bps, 改行コード: LF) で時刻設定をする
  - 時刻の取得: `get` (Enterキー)
  - 時刻の設定: `set 20XX/XX/XX XX:XX:XX` (Enterキー)
  - 時刻設定が完了すると赤色LEDが消灯するので、リセットする
